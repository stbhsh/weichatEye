...
- (void)viewDidLoad {
    ...
    self.weichatTble = [[UITableView alloc]initWithFrame:self.frame style:UITableViewStylePlain];
    self.weichatTble.delegate =self;
    self.weichatTble.dataSource= self;
    
    
//    self.weichatTble.contentOffset = 
    [self.view  addSubview:self.weichatTble];

    // Do any additional setup after loading the view, typically from a nib.
}
-(void)animationwith:(CGFloat)y
{
    CGFloat flag =self.frame.origin.y *2.0f -20.f;
    if (y < flag) {
    ...
    if (y < flag -20) {
        if (self.eyeballLayer.opacity <= 1.0f) {
            self.eyeballLayer.opacity +=1.0f;
        }
    }
